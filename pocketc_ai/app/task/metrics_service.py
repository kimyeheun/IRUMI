...
# -- GROUP BY 절을 SELECT 절의 표현식과 동일하게 수정
# GROUP BY t.user_id, t.sub_id, DATE(CONVERT_TZ(t.transacted_at,'UTC','Asia/Seoul'))
# ON DUPLICATE KEY UPDATE
#   -- ON DUPLICATE KEY UPDATE에서는 VALUES() 함수가 집계된 값을 참조하지 못할 수 있습니다.
#   -- 집계된 값을 사용하려면 별도의 로직이 필요하지만, 여기서는 키 중복 시 아무 작업도 하지 않도록 비워두거나
#   -- 실제 업데이트할 값을 명시해야 합니다. 아래는 키 업데이트 구문을 수정합니다.
